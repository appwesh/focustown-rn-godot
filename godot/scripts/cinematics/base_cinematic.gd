extends Node3D
class_name BaseCinematic

## Base cinematic scene controller
## Manages character spawning, animation, and cinematic sequences
## Supports tap-to-move and focus session functionality
##
## Subclasses should override:
## - _get_scene_name() - for logging
## - _get_building_id() - for study spot building_id
## - _get_building_name() - for study spot building_name
## - _get_default_npc_configs() - scene-specific NPC positions
...
@export var character_spawn_position: Vector3 = Vector3(0, 0, 5)
...
## Auto-create overview camera if none exists in scene
@export var auto_create_overview_camera: bool = true
## Overview camera position (high up and behind the scene)
@export var overview_camera_position: Vector3 = Vector3(0, 14.63, 29.524)
## Overview camera rotation in degrees (tilted down 30°, facing backward)
@export var overview_camera_rotation: Vector3 = Vector3(-30, 180, 0)
## Overview camera field of view (18 = zoomed in like library)
@export var overview_camera_fov: float = 18.0
...
## Return the scene name for logging (e.g., "Library", "Coastal")
func _get_scene_name() -> String:
	return "Base"


## Return the building ID for study spots (e.g., "library", "coastal")
func _get_building_id() -> String:
	return "base"


## Return the display name for the building (e.g., "Library", "Coastal Cafe")
func _get_building_name() -> String:
	return "Base"


## Register this scene with RNBridge - override in subclass
func _register_with_rnbridge() -> void:
	# Override in subclass to call specific registration method
	pass


## Return default NPC configurations for this scene
## Override in subclass to provide scene-specific positions
func _get_default_npc_configs() -> Array[Dictionary]:
	return []


## Return default study spot configurations for this scene
## Override in subclass to provide scene-specific chair positions
...
func _ensure_overview_camera() -> void:
	## Ensure we have an overview camera - use existing if available, create if not
	
	# Check if there's already a camera in the scene
	var existing_camera := get_viewport().get_camera_3d()
	if existing_camera:
		raycast_camera = existing_camera
		print("[%sCinematic] Using existing camera: %s" % [_get_scene_name(), existing_camera.name])
		return
	
	# No camera exists - create one if auto-create is enabled
	if not auto_create_overview_camera:
		push_warning("[%sCinematic] No camera found and auto_create_overview_camera is disabled" % _get_scene_name())
		return
	
	# Create overview camera
	var camera := Camera3D.new()
	camera.name = "OverviewCamera"
	camera.position = overview_camera_position
	camera.rotation_degrees = overview_camera_rotation
	camera.fov = overview_camera_fov
	camera.current = true
	
	add_child(camera)
	raycast_camera = camera
	print("[%sCinematic] Created overview camera at %s" % [_get_scene_name(), overview_camera_position])
...
func _spawn_npc_characters() -> void:
	...
	## First checks CinematicStudySpot nodes with spawn_npc=true, then falls back to code configs
	var configs: Array[Dictionary] = []
	
	# Try to get NPC configs from study spot nodes first
	var spot_nodes := _find_study_spot_nodes()
	for spot_node in spot_nodes:
		var npc_config := spot_node.get_npc_config()
		if not npc_config.is_empty():
			configs.append(npc_config)
	
	# If no NPCs from spots, use scene config or defaults
	if configs.is_empty():
		if not npc_configs.is_empty():
			configs.assign(npc_configs)
		else:
...
func _add_study_bubble_deferred(npc: CinematicCharacter) -> void:
	...
	print("[%sCinematic] Added study bubble to: %s" % [_get_scene_name(), npc.name])
...
func _apply_npc_preset_deferred(npc: CinematicCharacter, preset_name: String) -> void:
	...
	if not modular:
		push_warning("[%sCinematic] ModularCharacter not ready for: %s" % [_get_scene_name(), preset_name])
		return
	...
	if preset_name.is_empty():
		...
		print("[%sCinematic] Applied NPC skin: %s" % [_get_scene_name(), skin_name])
	else:
		...
		if not data.is_empty():
			...
			print("[%sCinematic] Applied NPC preset: %s" % [_get_scene_name(), preset_name])
		else:
			npc.randomize_appearance()
			print("[%sCinematic] Randomized NPC (preset not found): %s" % [_get_scene_name(), preset_name])
...
func _setup_camera_rig() -> void:
	...
	print("[%sCinematic] Camera rig initialized" % _get_scene_name())


func _on_camera_changed(camera_name: String) -> void:
	print("[%sCinematic] Camera changed to: %s" % [_get_scene_name(), camera_name])
...
func _start_entrance_cinematic():
	...
	print("[%sCinematic] Starting entrance cinematic" % _get_scene_name())
	...
	if not entrance_arrival_animation.is_empty():
		...
		print("[%sCinematic] Playing arrival animation: %s" % [_get_scene_name(), entrance_arrival_animation])
	...
	print("[%sCinematic] Entrance cinematic complete" % _get_scene_name())


## Trigger entrance cinematic manually (can be called from RN or code)
func trigger_entrance_cinematic() -> void:
	if not _character:
		push_warning("[%sCinematic] Cannot play entrance cinematic - no character" % _get_scene_name())
		return
...
func _find_study_spot_nodes() -> Array[CinematicStudySpot]:
	## Find all CinematicStudySpot nodes in the scene tree (children of this node or deeper)
	var spots: Array[CinematicStudySpot] = []
	_find_study_spot_nodes_recursive(self, spots)
	return spots


func _find_study_spot_nodes_recursive(node: Node, spots: Array[CinematicStudySpot]) -> void:
	## Recursively search for CinematicStudySpot nodes
...
func _on_study_spot_tapped(spot: Dictionary) -> void:
	...
	print("[%sCinematic] Walking to study spot: %s" % [_get_scene_name(), spot.get("name", "Unknown")])
...
func _trigger_session_at_spot(spot: Dictionary) -> void:
	...
	print("[%sCinematic] Arrived at study spot: %s" % [_get_scene_name(), spot.get("name", "Unknown")])
...
func _notify_rn_player_seated(spot: Dictionary) -> void:
	...
	var building_id: String = spot.get("building_id", _get_building_id())
	var building_name: String = spot.get("building_name", _get_building_name())
	...
	print("[%sCinematic] Notified RN: seated at %s in %s" % [_get_scene_name(), spot_id, building_name])
...
func start_focus_session() -> void:
	...
	print("[%sCinematic] Focus session started" % _get_scene_name())


## End focus session and return to idle
func end_focus_session() -> void:
	print("[%sCinematic] Ending focus session" % _get_scene_name())
	...
	if FocusSessionManager.is_focusing():
		...
		print("[%sCinematic] Focus session ended, earned %d coins" % [_get_scene_name(), coins])
...
func _on_focus_session_started(_spot: Node3D) -> void:
	print("[%sCinematic] Focus session started - playing studying animation" % _get_scene_name())
...
func _on_focus_session_ended(_duration: int, _coins: int) -> void:
	print("[%sCinematic] Focus session ended - returning to sitting idle" % _get_scene_name())
	if _character and _is_player_seated:
...
func _exit_first_person() -> void:
	...
	print("[%sCinematic] Exiting first person mode" % _get_scene_name())
...
func _apply_character_preset(preset_name: String) -> void:
	...
	if not _user_character_data.is_empty():
		...
		print("[%sCinematic] Applied user character data instead of preset" % _get_scene_name())
		return
	...
	if not preset_data.is_empty():
		...
		print("[%sCinematic] Applied preset: %s" % [_get_scene_name(), preset_name])
	else:
		# Create random if preset not found
		push_warning("[%sCinematic] Preset '%s' not found, using random" % [_get_scene_name(), preset_name])
...
func set_user_character(skin_data: Dictionary) -> void:
	...
	print("[%sCinematic] User character data received: %s" % [_get_scene_name(), skin_data.keys()])
...
func _apply_user_character_deferred(data: Dictionary) -> void:
	...
	if not modular:
		push_warning("[%sCinematic] ModularCharacter not ready for user" % _get_scene_name())
		return
	
	_character.apply_preset_dict(data, "Player")
	print("[%sCinematic] Applied user character data" % _get_scene_name())
...
func play_celebration_animation() -> void:
	if _character and _is_player_seated:
		print("[%sCinematic] Playing celebration animation" % _get_scene_name())
